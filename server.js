...
app.post("/api/parse", async (req, res) => {
  try {
    ...
    if (items.length === 0) {
      console.log(
        "❌ Nenhum item encontrado. HTML pode ter captcha ou layout diferente.",
      );
      return res.status(400).json({
        error:
          "Nenhum item encontrado na página da nota. A Sefaz pode ter bloqueado a consulta.",
...
app.post("/api/categorize", async (req, res) => {
  try {
    ...
    res.json({
      items: (req.body.items || []).map((i) => ({ ...i, category: "Geral" })),
    });
  }
});
...
// 🔄 Rota: Restaurar backup completo (apaga tudo e reinsere)
app.put("/api/receipts/restore", async (req, res) => {
  try {
    const { receipts } = req.body;
    if (!receipts || !Array.isArray(receipts)) {
      return res
        .status(400)
        .json({ error: "Dados inválidos: receipts deve ser um array" });
    }
    await restoreAllReceipts(receipts);
    res.json({ success: true, restored: receipts.length });
  } catch (err) {
